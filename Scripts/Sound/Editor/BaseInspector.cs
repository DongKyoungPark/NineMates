...
public class BaseInspector : Editor
{
    ...
    {
        //+ 시작 타입을 지정.
        p_Target.eventStartType = (EventStartType)EditorGUILayout.EnumPopup("Start type", p_Target.eventStartType);
        //+ 만약 마우스 클릭이나 터치가 시작타입이라면 클릭영역 크기를 지정.
        if (EventStartType.INTERACT.Equals(p_Target.eventStartType))
        {
        ...
        else if (EventStartType.KEY_PRESS.Equals(p_Target.eventStartType))
        {
